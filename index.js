...
atem_state=[];
inTransition=false;
obs_data.sceneName=config.scenes[0].sceneName;
...
for(let i = 0; i<config.scenes.length; i++) {
	scenes[i]= {};
	scenes[i].config = config.scenes[i];
}
...
function update(data, state) {
	...
	for(let j = 0; j < scenes.length; j++){
		if(obs_data.sceneName===scenes[j].sceneName){
			for(let i = 0; i < tallies.length; i++) {
				...
				if(state.video.ME[0].fadeToBlack && state.video.ME[0].fadeToBlack.isFullyBlack) {
					tallies[i].lights.color(scenes[j].ftbColor);
					//console.log("ftb");
					...
					if(!inTransition){
						tallies[i].lights.color(scenes[j].fadeInColor);
						//console.log(i+" yellow");
					}
				} else if(state.video.ME[0].inTransition && (program === tallies[i].config.inputID)){
					if(!inTransition){
						tallies[i].lights.color(scenes[j].fadeOutColor);
						//console.log(i+" red");
					} 
				} else if(program === tallies[i].config.inputID) {
					tallies[i].lights.color(scenes[j].programColor);
					//console.log(i+" green");
				} else if(preview === tallies[i].config.inputID) {
					tallies[i].lights.color(scenes[j].previewColor);
